...
class main (qt.QMainWindow):
    def __init__(self):
        ...
        self.city.setAccessibleName(_("city name"))
        layout.addWidget(self.city)
        self.getResult=qt.QPushButton(_("get weather data"))
        self.getResult.setDefault(True)
        self.getResult.clicked.connect(self.on_get)
        layout.addWidget(self.getResult)
        self.result=qt.QListWidget()
        self.result.setAccessibleName(_("weather results"))
    ...
    def closeEvent(self, event):
        if settings_handler.get("g","exitDialog")=="True":
            ...
            self.close()
    def on_get(self):
        url = f'http://api.openweathermap.org/data/2.5/weather?q={self.city.text()}&appid={api_key}'
        response = requests.get(url)
        if response.status_code==200:
            data = response.json()
            temperature_celsius = str(data['main']['temp']- 273.15).split(".")[0]
            weather_conditions = data['weather'][0]['main']  # Get the main weather condition
            wind_speed = data['wind']['speed']
            high_temperature_celsius = str(data['main']['temp_max']- 273.15).split(".")[0]
            low_temperature_celsius = str(data['main']['temp_min']- 273.15).split(".")[0]
            humidity= str(data['main']['humidity'])
            self.result.clear()
            self.result.addItems([_("temperature : {}").format(temperature_celsius),_("the weather is {}").format(weather_conditions),_("wend speed : {} /s").format(str(wind_speed)),_("high temperature : {}").format(high_temperature_celsius),_("low temperature : {}").format(low_temperature_celsius),_("humidity : {} %").format(str(humidity))])
